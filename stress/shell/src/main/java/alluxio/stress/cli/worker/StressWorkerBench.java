...
import alluxio.stress.worker.WorkerBenchOperation;
...
public class StressWorkerBench extends Benchmark<WorkerBenchTaskResult> {
  ...
  private final class BenchContext {
    ...
    public synchronized WorkerBenchTaskResult getResult() {
      return mResult;
    }
  ...
  private final class BenchThread implements Callable<Void> {
    ...
    private final WorkerBenchTaskResult mResult = new WorkerBenchTaskResult();
    ...
    private void runInternal() throws Exception {
      ...
      if (mParameters.mOperation == WorkerBenchOperation.Prepare) {
    ...
    private int applyOperation() throws IOException {
      if (mParameters.mOperation == WorkerBenchOperation.Read) {
        if (mInStream == null) {
